...
class SessionService {
  ...
  /// GET THERAPY CENTER
  static Stream<List<TherapyCenterDataModel>> getTherapyCenterLocationData() {
    return CollectionUtils.therapyCenterCollection
        .where('userId', isEqualTo: SharedPreferenceUtils.getUserId())
        .snapshots()
        .map((event) => event.docs.map((e) {
              return TherapyCenterDataModel.fromJson(e.data());
            }).toList());
  }

  /// Update isFeatchCenter
  static Future<bool> updateIsFeatchTherapyCenter(String id) {
    return CollectionUtils.therapyCenterCollection
        .doc(id)
        .update({"isFeatchCenter": true})
        .then((value) => true)
        .catchError((e) {
          logs(' delete ERROR :=>$e');
          return false;
  ...
  /// GET THERAPY CENTER FOR DROP DOWN LIST
  Future<List<TherapyCenterDataModel>> getTherapyDropdownCenter() async {
    try {
      final snapshot = await CollectionUtils.therapyCenterCollection.get();
      ...
      if (snapshot.docs.isNotEmpty) {
        return snapshot.docs.map((doc) {
          ...
          return TherapyCenterDataModel.fromJson(data);
        }).toList();
      } else {
        return []; // Return an empty list if no data found
      }
